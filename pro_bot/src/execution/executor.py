...
class Executor:
    ...
    @staticmethod
    def _format_step(value: float, step: float) -> str:
        s = str(step)
        if "." in s:
    ...
    async def get_balance_usdt(self) -> float:
        if self.settings.mode == "paper":
            return 1000.0
        # Try unified, then spot
        try:
            bal = await self.client.fetch_balance("UNIFIED")
        except Exception:
            bal = await self.client.fetch_balance("SPOT")
    ...
    async def handle_symbol(self, symbol: str) -> None:
        try:
            ...
            # Align price and qty to exchange steps
            price = self._round_to_step(signal.entry_price, rules.price_step, mode="floor")
            qty = self._round_to_step(qty, rules.qty_step, mode="floor")

            # Validate against min qty/notional after rounding
            if qty < rules.min_qty:
                return
            if qty * price < rules.min_notional:
                return

            await self.notifier.send_trade(symbol, signal.side, qty, price, signal.reason)

            if self.settings.mode == "paper":
                # Simulate instant fill at best ask
                self.positions[symbol] = Position(symbol, qty, price, signal.sl_price, signal.tp_price)
            ...
            try:
                res = await self.client.place_order(
                    ...
                    side=signal.side,
                    order_type="Limit",
                    qty=self._format_step(qty, rules.qty_step),
                    price=self._format_step(price, rules.price_step),
