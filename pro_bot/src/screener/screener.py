...
class Screener:
    ...
    async def fetch_candidates(self) -> List[ScreenedSymbol]:
        ...
        symbols_all = [s["symbol"] for s in symbols_info if s.get("quoteCoin") == "USDT"]
        # Ticker volumes
        tickers = await self.client.fetch_ticker_24h()
        vol24 = {t["symbol"]: float(t.get("turnover24h", 0)) for t in tickers}

        # Exclude stablecoin-stablecoin and exotic synthetic pairs
        def is_excluded(sym: str) -> bool:
            if not sym.endswith("USDT"):
                return True
            base = sym[:-4]
            excluded_bases = {"USDT", "USDC", "DAI", "USDD", "USDE", "USD1", "PYUSD", "USDTB", "USDQ"}
            return base in excluded_bases or "VIRTUAL" in sym or "TEST" in sym

        # Keep only liquid top-N by 24h turnover to respect rate limits
        symbols_ranked = sorted((s for s in symbols_all if not is_excluded(s)), key=lambda s: vol24.get(s, 0.0), reverse=True)
        symbols = symbols_ranked[:80]
        ...
        # Concurrency limited (reduce to ease rate limits)
